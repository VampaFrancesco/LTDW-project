...
?>
                            ...
                            <div class="order-details">
                                ...
                                <div class="order-actions">
                                    <button type="button" class="btn-view-details"
                                            onclick="showOrderDetails('<?php echo htmlspecialchars($order['id']); ?>')">
                                        <i class="bi bi-eye"></i> Vedi Dettagli
                                    </button>

                                    <button type="button" class="btn-buy-again"
                                            onclick="buyAgain('<?php echo htmlspecialchars($order['id']); ?>')">
                                        <i class="bi bi-cart-plus"></i> Compra di nuovo
                                    </button>

                                    <?php if ($order['raw_status'] == 2 && !empty($order['tracking'])): ?>
                                        <a href="track_order.php?id=<?php echo htmlspecialchars($order['id']); ?>" class="btn btn-outline-primary">
    ...
    <div class="modal fade" id="productNotFoundModal" tabindex="-1" aria-labelledby="productNotFoundModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="productNotFoundModalLabel">
                        <i class="bi bi-exclamation-triangle text-warning"></i> Prodotto Non Disponibile
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body text-center">
                    <p class="mb-3">Prodotto non trovato o esaurito</p>
                    <p class="text-muted">Il prodotto che stai cercando di acquistare nuovamente non è più disponibile nel nostro catalogo.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Chiudi</button>
                </div>
    ...
    <script>
        ...
        // Modifica qui: Accetta un ID come parametro e cerca l'oggetto corrispondente
        function buyAgain(orderId) {
            ...
            // Verifica in tempo reale la disponibilità del prodotto
            fetch('../action/check_product_availability.php', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (!data.is_available || !data.product_url) {
                    showProductNotFound();
                    return;
                }

                // Costruisce l'URL di destinazione con l'ID del prodotto
                let targetUrl = data.product_url;
                
                // Aggiunge il parametro per l'ID del prodotto e per fare scroll al prodotto specifico
                if (order.product_type === 'mystery_box') {
                    targetUrl += `?highlight_box=${data.product_id}`;
                } else if (order.product_type === 'oggetto') {
                    targetUrl += `?highlight_item=${data.product_id}`;
                }
        ...
        function showProductNotFound() {
            const modal = new bootstrap.Modal(document.getElementById('productNotFoundModal'));
            modal.show();
        }
